			...
			BuildInfo? info = null)
		{
			...
			if (!await LicenseCheck.HasValidLicense())
			{
				if (NeedleEngineAuthorization.TrialEnded)
				{
					...
					trialLicenseMessage =
						"<b>Needle Engine Free Trial has ended</b>: Please purchase a license to continue using Needle Engine at " +
				...
				else
				{
					...
					{
						trialLicenseMessage = "<b>Needle Engine Trial period ends in " + daysUntilEndOfTrial +
						                      " days</b>. Please purchase a license to continue using Needle Engine at " +
						                      Constants.BuyLicenseUrl.AsLink();
					}
				}
			}
