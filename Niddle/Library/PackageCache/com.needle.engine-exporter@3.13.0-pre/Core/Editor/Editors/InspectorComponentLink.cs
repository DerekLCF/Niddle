					...
					{					
						...
						if (timeSinceLastClick.TotalSeconds > .5f && packageJsonDirectory != null && packageJsonDirectory.EndsWith("~"))
						{
							...
							if (File.Exists(npmdefPath))
							{
								...
								if (asset)
								{
									Debug.Log("The NpmDef containing this script is not installed in your current web project. Add it to ExportInfo dependencies list to install it: " + asset, asset);
